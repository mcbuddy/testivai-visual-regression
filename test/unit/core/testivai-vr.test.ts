/**
 * Tests for TestivaiVR main class
 */

import { TestivaiVR } from '../../../src/core/testivai-vr';
import { Plugin, TestivaiVROptions } from '../../../src/core/interfaces';
...
describe('TestivaiVR', () => {
  describe('init', () => {
    it('should create a new instance with default options', () => {
      ...
      const instance = TestivaiVR.init(options);
      
      expect(instance).toBeInstanceOf(TestivaiVR);
  ...
  describe('use', () => {
    ...
    it('should throw an error if plugin framework does not match', () => {
      ...
      expect(() => {
        instance.use(mockPlugin);
      }).toThrow('Plugin cypress-plugin is for framework cypress, but TestivaiVR is configured for playwright');
    });
  });
